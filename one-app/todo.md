...
## PATCH: Ambiguity Handling Layer (Phase 2A Kernel Enhancement)

### Kernel Extension
- [x] Add CLARIFY to decision_outcome enum (ALLOW | DENY | REQUIRE_HUMAN | CLARIFY)
- [x] Implement confidence_score() function: field_completeness*0.4 + pattern_grounding*0.3 + prior_context_match*0.2 + human_signature_present*0.1
- [x] Add 6 ambiguity detection rules to kernel evaluation (missing field, conflicting fields, low confidence, no grounding, scope underspecified, time ambiguous)
- [x] CLARIFY output includes: reason, missing_fields, confidence_signals_available

### Clarification Agent
- [x] Create clarification_agent service (stateless, non-authoritative, no memory)
- [x] Agent emits clarify_requested event with questions, TTL (180s default, configurable 120-300s)
- [x] Agent processes human clarify_response and produces refined_packet (linked via parent_packet_id)
- [x] TTL enforcement: timeout emits clarify_timeout event then REQUIRE_HUMAN (NO defaults, NO assumptions)
- [x] Max 3 rounds per packet, max 15 minutes total
- [x] NO-FALLBACK RULE: silence = escalate, never assume (HARD CONSTRAINT)

### Packet Schema and Events
- [x] Event types: clarify_requested, clarify_response, clarify_timeout
- [x] Refined packet schema: packet_id suffixed _refined_rN, parent_packet_id, clarification_round
- [x] Original packet NEVER mutated (new packet created, original preserved for audit)
- [x] All events logged to mailbox with trace_id linkage

### 9 Tests (per PATCH document)
- [x] Test 1: Missing required field triggers CLARIFY (3 sub-tests)
- [x] Test 2: Low confidence model input triggers CLARIFY (3 sub-tests)
- [x] Test 3: Clarification agent cannot execute (3 sub-tests: code audit + structural)
- [x] Test 4: Linked pair maintains lineage (2 sub-tests: single + multi-round)
- [x] Test 5: Timeout escalates with NO defaults applied (3 sub-tests)
- [x] Test 6: No-defaulting constraint (2 sub-tests: code audit + behavioral)
- [x] Test 7: Original packet never mutated (2 sub-tests: byte-for-byte + separate object)
- [x] Test 8: Multiple clarification rounds (6 sub-tests: max 3 + 15min total)
- [x] Test 9: Timeout prevents execution (3 sub-tests: routed not executed + POLICY_VIOLATION)
- [x] All 35/35 tests pass — 9 core tests + 6 ambiguity detection rules + 2 question generation

## Portable Skill System (Apr 15)

- [x] _invariants.md: 12 canonical invariants (INV-001 through INV-012), 6 ERR_FATAL codes
- [x] _patches.md: patch history document with version tracking
- [x] governance/skill.md: mandatory governance skill (load_order: 0, required: true, SHA-256 hash)
- [x] builder/skill.md: builder role skill (propose, no execute, no approve)
- [x] clarification/skill.md: clarification role skill (NO-FALLBACK, stateless, non-authoritative, TTL, max 3 rounds)
- [x] witness/skill.md: witness role skill (observe only, no block, no modify, sentinel mailbox)
- [x] connector/skill.md: connector role skill (translate only, no direct API, no credential storage)
- [x] adapters/_framework.md: universal adapter contract (governance_hash, no authority addition, no constraint removal)
- [x] adapters/manus.md: Manus adapter (SKILL.md format, submit_proposal tool mapping)
- [x] adapters/openai.md: OpenAI adapter (system message, function calling)
- [x] adapters/claude.md: Claude adapter (XML-tagged system prompt, tool use)
- [x] adapters/gemini.md: Gemini adapter (system_instruction, function declarations)
- [x] skillSystem.test.ts: 148 constraint enforcement validation tests (10 proofs)
