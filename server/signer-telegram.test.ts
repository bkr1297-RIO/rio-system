...
describe("Signer Management", () => {
  ...
  it("listSigners rejects non-owner callers", async () => {
    const caller = appRouter.createCaller(createTestContext(1, "not-the-owner"));
    await expect(caller.proxy.listSigners()).rejects.toThrow("Only the system owner");
  });
...
describe("Telegram Module", () => {
  it("isTelegramConfigured returns false when no credentials", async () => {
    const { isTelegramConfigured } = await import("./telegram");
    // ENV.telegramBotToken and ENV.telegramChatId are empty strings by default
